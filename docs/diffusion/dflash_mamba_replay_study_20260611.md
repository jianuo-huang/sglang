...
- 2026-06-11 follow-up 已修复 partial-K CUDA graph correctness：`K=0/1/4/8` 在固定 prompt exact-output 中均与 `K=16` full-cache baseline 完全一致。
- 修复后 smoke 性能矩阵覆盖 `K in {0,1,4,8,16}`、`C in {1,4,16}`。同一并发下所有 K 的 accept length 完全一致，说明之前 `K=1/4/8` 的 accept drift 已消失。

因此当前分支已经有比较明确的显存证据、replay correctness 证据和修复后性能 smoke 数据。PR 前建议再补一轮完整 25 点矩阵作为最终性能表。
...
注意：本节是修复前在 commit `944bfc374` 采集的完整 25 点历史矩阵。`K=1/4/8` 的性能结果是在 correctness blocker 尚未修复时采集的，只能用于定位和趋势参考。修复后的 smoke 矩阵见后文。
...
- 当前 GDN/linear attention 的 `ForwardMetadata` 在 `_forward_metadata`、`_capture_metadata`、`_replay_metadata` 中没有写回 `draft_token_num`，导致 commit 阶段读到默认值 `1`。
- 因为 `update_mamba_state_after_mtp_verify` 用 `forward_metadata.draft_token_num` 判断是否启用 replay，`K=1/4/8` 被误判成非 reduced-cache replay，直接从只有 K 步的 intermediate cache 按 accepted step scatter。
- `K=0` 仍满足 `0 < 1`，所以走 zero-cache replay；`K=16` 是 full-cache，scatter 合法。因此修复前表现为 `K=0/K=16` 正常、`K=1/4/8` accept length 下降。
- 修复方式是在三条 metadata 构造路径中恢复 `is_target_verify` 和 `draft_token_num` 字段，使 partial-K 正确进入 replay commit 路径。
...
### 2026-06-11 metadata fix

代码改动：

- 文件：`python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`
- 在 `MambaAttnBackendBase._forward_metadata`、`_capture_metadata`、`_replay_metadata` 的 `ForwardMetadata(...)` 中补回 `is_target_verify` 和 `draft_token_num`。
- 这是 GDN/linear backend 的 metadata 修复，不涉及 Domino/projector，也不改变用户参数。

静态检查：

```text
git diff --check: pass
py_compile hybrid_linear_attn_backend.py gdn_backend.py fused_sigmoid_gating_recurrent.py: pass
```

CUDA graph exact-output correctness：

```text
artifact: /mnt/cpfs/zyj/casual_spec/bench_outputs/dflash_mamba_replay_20260611/correctness_metadata_fix
```

结果：

| K | baseline | prompts | result |
| ---: | ---: | ---: | --- |
| 0 | 16 | 4 | exact match |
| 1 | 16 | 4 | exact match |
| 4 | 16 | 4 | exact match |
...
```text
artifact: /mnt/cpfs/zyj/casual_spec/bench_outputs/dflash_mamba_replay_20260611/matrix_metadata_fix_smoke
num_prompts=40, random_input_len=256, random_output_len=64
K in {0,1,4,8,16}, C in {1,4,16}
```

| K | C | output TPS | mean TTFT ms | mean TPOT ms | mean E2E ms | accept length |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 1 | 98.23 | 99.04 | 8.75 | 650.52 | 3.870 |
| 0 | 4 | 252.87 | 151.85 | 13.55 | 1005.29 | 3.947 |
| 0 | 16 | 463.89 | 383.95 | 25.13 | 1966.98 | 4.087 |
| 1 | 1 | 101.08 | 99.08 | 8.47 | 632.51 | 3.870 |
| 1 | 4 | 240.91 | 146.18 | 14.44 | 1055.60 | 3.947 |
| 1 | 16 | 364.11 | 433.20 | 31.65 | 2427.00 | 4.087 |
| 4 | 1 | 107.16 | 96.60 | 7.94 | 596.56 | 3.870 |
| 4 | 4 | 228.00 | 151.87 | 14.72 | 1079.27 | 3.947 |
| 4 | 16 | 361.00 | 419.14 | 32.27 | 2451.92 | 4.087 |
| 8 | 1 | 106.97 | 98.71 | 7.91 | 597.22 | 3.870 |
| 8 | 4 | 241.29 | 145.50 | 14.42 | 1054.10 | 3.947 |
| 8 | 16 | 365.80 | 380.26 | 32.41 | 2421.81 | 4.087 |
| 16 | 1 | 107.70 | 97.53 | 7.87 | 593.55 | 3.870 |
| 16 | 4 | 262.19 | 145.86 | 13.08 | 969.92 | 3.947 |
| 16 | 16 | 431.63 | 461.36 | 26.27 | 2116.39 | 4.087 |

关键结论：

- 同一并发下，所有 K 的 accept length 完全一致，修复前的 partial-K accept drift 已消失。
- `K=1/4/8` 的 TPS/latency 现在可以作为有效 smoke 数据解读。
- 这轮是 15 点 smoke 矩阵，样本数小于前面的 25 点完整矩阵；最终 PR 建议再补 `C in {1,2,4,8,16}`、`num_prompts=80` 的完整矩阵。
...
- partial-K replay (`K=1/4/8`) 在 CUDA graph exact-output 对比中已经通过。
- 修复后 15 点 smoke benchmark 已完成，accept length 已对齐；修复前 25 点完整矩阵仍可用于显存和历史诊断参考。

建议 PR 策略：

1. 保留当前最小 metadata 修复，不引入 Domino 或 draft projector 改动。
2. 提 PR 前再跑一次完整 `K in {0,1,4,8,16}`、`C in {1,2,4,8,16}` 矩阵，替换修复前的 TPS 表。
3. PR 正文中同时给出 memory/capacity 表、exact-output correctness 表、修复后 benchmark 表。
