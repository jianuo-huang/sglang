...
class TestDFlashMambaReplayArgs(CustomTestCase):
    ...
    def test_verify_reduced_mamba_cache_requires_replay(self):
        ...
        spec_info = DFlashVerifyInput(
            draft_token=torch.arange(batch_size * draft_token_num, dtype=torch.int64),
            positions=torch.arange(batch_size * draft_token_num, dtype=torch.int64),
        ...
        logits_output = SimpleNamespace(
            next_token_logits=torch.zeros(batch_size * draft_token_num, 8),
            hidden_states=None,
        )

        with self.assertRaisesRegex(RuntimeError, "requires replay"):
            spec_info.verify(batch=batch, logits_output=logits_output, page_size=1)
