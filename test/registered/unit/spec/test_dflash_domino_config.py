...
class TestDFlashDominoConfigParsing(CustomTestCase):
    ...
    def test_nested_emb_dim_fallback(self):
        cfg = _domino_dflash_config()
        cfg["emb_dim"] = None
        cfg["dflash_config"]["emb_dim"] = 384
        parsed = parse_dflash_draft_config(draft_hf_config=cfg)
        self.assertEqual(parsed.emb_dim, 384)
    ...
    def test_other_projector_type_is_preserved_for_shared_parser_users(self):
        cfg = _domino_dflash_config()
        cfg["dflash_config"]["projector_type"] = "dspark"
        parsed = parse_dflash_draft_config(draft_hf_config=cfg)
        self.assertEqual(parsed.projector_type, "dspark")
        self.assertFalse(is_dflash_domino_projector(parsed.projector_type))
